...
static int wireless_simu_pci_probe(struct pci_dev *pdev, const struct pci_device_id *pci_id_dev)
{
    ...
    wireless_hif_write32(priv, HAL_BASIC_REG(WIRELESS_REG_BASIC_IRQ_ENABLE), 1);
    ...
    // ce alloc
    ret = wireless_simu_ce_alloc_pipes(priv);
    if (ret)
    {
        goto End;
    }
    pr_info("%s : ce alloc success \n", WIRELESS_SIMU_DEVICE_NAME);
