...
static struct wireless_simu* simu_priv = NULL;
					  ...
					  struct ieee80211_channel *chan)
{
	...
	if (ieee80211_is_beacon(hdr->frame_control) ||
		...
		pr_info("%s : not in rx_isbeacon", WIRELESS_SIMU_DEVICE_NAME);
	}

	pr_info("%s : rx_hwsim success 9", WIRELESS_SIMU_DEVICE_NAME);
	...
	list_for_each_entry(data2, &hwsim_radios, list) {
		...
		if (data == data2){
			pr_err("%s : hw_simu data same error", WIRELESS_SIMU_DEVICE_NAME);
		}
...
static void build_rx_status(struct ieee80211_hw *hw,
					  struct sk_buff *skb,
					  struct ieee80211_channel *chan, 
					  struct ieee80211_rx_status *rx_status)
{
...
bool mac80211_hwsim_tx_frame_layer(struct ieee80211_hw *hw,
					  struct ieee80211_rx_status *rx_status,
					  struct sk_buff *skb)
{
	mac80211_hwsim_rx(hw->priv, rx_status, skb);
...
static void mac80211_hwsim_tx(struct ieee80211_hw *hw,
			      ...
			      struct sk_buff *skb)
{
	pr_info("mac80211_hwsim: TX frame\n");
	struct mac80211_hwsim_data *data = hw->priv;
	...
	if (simu_priv) {
		// 保留原有的skb->cb 这个大小为 48 字节
		char magic = 0xaf;
		int channel_size = sizeof(struct ieee80211_channel);
		struct ieee80211_rx_status rx_status;
		build_rx_status(hw, skb, channel, &rx_status);
		int num_byte_pad = ARRAY_SIZE(skb->cb) * sizeof(skb->cb[0]) + sizeof(magic) + channel_size + sizeof(rx_status);
		struct sk_buff* skb_new = skb_copy_expand(skb, skb_headroom(skb), num_byte_pad, GFP_KERNEL);
		if (skb->sk != NULL) {
			skb_set_owner_w(skb_new, skb->sk);
		}
		dev_kfree_skb(skb);
		skb = skb_new;
		pr_info("%s : tx new skb origin_len %d", WIRELESS_SIMU_DEVICE_NAME, skb->len);
		
		skb_put_data(skb, &magic, sizeof(magic));
		skb_put_data(skb, &rx_status, sizeof(rx_status));
		skb_put_data(skb, channel, channel_size);
		skb_put_data(skb, skb->cb, ARRAY_SIZE(skb->cb) * sizeof(skb->cb[0]));

		struct wireless_skb_cb *skb_cb = WIRELESS_SKB_CB(skb);
		int ret = 1;
		memset(skb_cb, 0, sizeof(*skb_cb));
		skb_cb->paddr = dma_map_single(&simu_priv->pci_dev->dev, skb->data, skb->len, DMA_TO_DEVICE);
		ret = wireless_simu_ce_send(simu_priv, skb, 2, 0);
		if (ret != 0)
		{
			pr_info("%s : simu_simple tx send err %d \n", WIRELESS_SIMU_DEVICE_NAME, ret);
			dma_unmap_single(&simu_priv->pci_dev->dev, skb_cb->paddr, skb->len, DMA_TO_DEVICE);
			ieee80211_tx_info_clear_status(txi);
			ieee80211_tx_status_irqsafe(hw, skb);
			return;
		}

		pr_info("%s : pkt tx succeed , skb len %d\n", WIRELESS_SIMU_DEVICE_NAME, skb->len);
		return;
	}
...
int init_mac80211_hwsim(struct wireless_simu* priv)
{
	int i, err;
	simu_priv = priv;
