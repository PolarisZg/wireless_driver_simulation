...
static int wireless_simu_wmi_cmd_send_nowait(struct wireless_simu *priv, struct sk_buff *skb, u32 cmd_id)
{
...
int wireless_simu_wmi_mgmt_send(struct wireless_simu *priv, u32 vif_id, u32 buf_id, struct sk_buff *frame)
{
    ...
    /* 和 64 取最小值, 小于 64 的时候直接装进 wmi_tlv 之中
     * 过大则放在外面
     */
    ...
    cmd->paddr_lo = lower_32_bits(WIRELESS_SKB_CB(frame)->paddr);
    cmd->paddr_hi = upper_32_bits(WIRELESS_SKB_CB(frame)->paddr);
    cmd->frame_len = frame->len;
    cmd->buf_len = buf_len;
    cmd->tx_params_valid = 0x1234;

    frame_tlv = (struct wmi_tlv *)(skb->data + sizeof(*cmd));
    frame_tlv->header = FIELD_PREP(WMI_TLV_TAG, WMI_TAG_ARRAY_BYTE) | FIELD_PREP(WMI_TLV_LEN, buf_len);
    memcpy(frame_tlv->value, frame->data, buf_len);

    wireless_simu_ce_byte_swap(frame_tlv->value, buf_len);

    ret = wireless_simu_wmi_cmd_send(priv, skb, WMI_MGMT_TX_SEND_CMDID);
    if (ret)
    {
        pr_warn("%s : fail to submit WMI_SENG_CMD %d \n", WIRELESS_SIMU_DEVICE_NAME, ret);
        dev_kfree_skb(skb);
        return ret;
    }

    pr_info("%s : cmd mgmt tx send \n", WIRELESS_SIMU_DEVICE_NAME);
